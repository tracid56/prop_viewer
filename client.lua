...
Citizen.CreateThread(function()
	while true do
		...
		AddTextComponentString(propList[currentPropIndex] .. ' : ' .. currentPropIndex)
		DisplayHelpTextFromStringLabel(0, 0, 1, -1)

		if IsControlJustReleased(0, Keys['LEFT']) then
			if currentPropIndex > 1 then
				currentPropIndex = currentPropIndex - 1
			else
				currentPropIndex = #propList
			end
		end

		if IsControlJustReleased(0, Keys['RIGHT']) then
			if currentPropIndex < #propList then
				currentPropIndex = currentPropIndex + 1
			end
		end
		...
		if IsControlJustReleased(0, Keys['TOP']) then
			if currentPropIndex > 1 then
				currentPropIndex = currentPropIndex - 100
			else
				currentPropIndex = #propList
			end
		end
		...
		if IsControlJustReleased(0, Keys['ENTER']) then
			...
			local propHash = GetHashKey(propList[currentPropIndex])
			local playerCoords = GetEntityCoords(GetPlayerPed(-1))
			currentProp = CreateObject(propHash, playerCoords.x + 1, playerCoords.y, playerCoords.z, 1, true, true)
		end
	end
end)